...
H-12 sendMessage 502 → 맥락 표시 안 함 → M8          ✅ ✅ ✅
결과: 25 통과 / 0 실패
```

`RestartOnTrigger=false` 로 두면 H-6 이 건너뛰어져 **22 통과**가 나옵니다. 정상입니다.

**H-9 / H-10 / H-11 / H-12 는 한 묶음입니다.** `/api/sendMessage` 로 대화를 시작한 외부 시스템
(예: 2차인증 로봇)의 답변이 시작 안내(M8)에 흡수되지 않아야 하고(H-9), 그 예외는 TTL 로
닫혀야 하며(H-10), **그 엔드포인트를 쓰지 않는 배포의 동작은 하나도 바뀌지 않아야
합니다(H-11).** 발송이 실패했으면(502) 사용자는 아무것도 받지 못했으므로 맥락으로
취급하지 않아야 합니다(H-12). **H-11 이 다른 고객에 대한 회귀 방어선입니다** — 이 성질이
깨지면 고객별 설정 없이 단일 소스로 운영할 수 없게 됩니다.

H-12 는 mock 의 `{"teams":"fail"}` 모드를 씁니다. 대화 생성이 500 을 돌려주므로
`createConversationAndSendMessage` 가 false 를 반환하고 라우트가 502 가 됩니다.
...
404("그런 Job 없음")는 확정된 답이므로 정리하고 기동해야 하며, 그러지 않으면 오래된
Job ID 하나로 사용자가 영구 정지됩니다. 두 경우를 뭉뚱그리면 한쪽이 반드시 깨집니다.

시나리오는 실행마다 새 사용자 ID 를 만들므로 **앱을 재기동하지 않고 반복 실행해도**
같은 결과가 나옵니다(`jobtable` 은 앱 프로세스 안에 남습니다).
